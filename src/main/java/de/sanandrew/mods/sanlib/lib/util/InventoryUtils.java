...
public final class InventoryUtils
{
    ...
    public static boolean canStackFitInInventory(ItemStack is, IInventory inv, boolean checkNBT, int maxStackSize, int begin, int end) {
        ...
        for( int i = begin; i < end; i++ ) {
            ...
            if( ItemStackUtils.isValid(invIS) && ItemStackUtils.areEqual(is, invIS, checkNBT) ) {
                int fit = Math.min(invIS.getMaxStackSize(), maxStackSize) - invIS.stackSize;
                int stackCnt = stack.stackSize;
                if( fit >= stackCnt ) {
                    return true;
                } else {
                    stack.stackSize = (stackCnt - fit);
                }
        ...
        if( ItemStackUtils.isValid(is) && provider.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, facing) ) {
            ...
            for( int i = begin; i < end; i++ ) {
                ...
                if( !ItemStackUtils.isValid(maxStack) ) {
                    is.stackSize = (is.stackSize - maxStackSize);
                }
                if( is.stackSize <= 0 ) {
