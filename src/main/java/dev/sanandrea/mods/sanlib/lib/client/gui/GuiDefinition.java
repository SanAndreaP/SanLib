        ...
        implements ResourceManagerReloadListener, IGuiReference, IElementContainer
{
    ...
    void register() throws IOException {
        if( Minecraft.getInstance().getResourceManager() instanceof ReloadableResourceManager rMgr ) {
            rMgr.registerReloadListener(this);
    ...
    private void loadFile(ResourceLocation data) throws IOException {
        Optional<Resource> optRes = Minecraft.getInstance().getResourceManager().getResource(data);
        if( optRes.isEmpty() ) {
        ...
        try( InputStreamReader reader = new InputStreamReader(optRes.get().open(), StandardCharsets.UTF_8) )
        {
            ...
            if( jObj.has("parent") ) {
                this.loadFile(ResourceLocation.parse(JsonUtils.getStringVal(jObj.get("parent"))));
            }
    ...
    public GuiElement loadElement(String id, JsonObject data) {
        ...
        Function<String, GuiElement> s = TYPES.get(ResourceLocation.parse(type));
        if( s == null ) {
    ...
    public void drawBackground(IGui gui, GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        this.drawBackground(gui, graphics, 0, 0, mouseX, mouseY, partialTicks);
    }
    ...
    public void drawBackgroundContainer(IGui gui, GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        this.drawBackground(gui, graphics, gui.getPosX(), gui.getPosY(), mouseX, mouseY, partialTicks);
    }

    public void drawForeground(IGui gui, GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        this.drawForeground(gui, graphics, 0, 0, mouseX, mouseY, partialTicks);
    }
