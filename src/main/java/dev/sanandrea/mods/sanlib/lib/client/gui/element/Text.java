        ...
        extends GuiElement
{
    ...
    @Nonnull
    private BiFunction<IGui, Component, Component> getText = (gui, origText) -> origText;

    private       String                currColorId = DEFAULT_COLOR;
    private       int                   currColor;
    private       int                   prevColor;
    private       Component        prevText;
    private final List<FormattedText> renderedLines = new ArrayList<>();
    ...
    public void render(IGui gui, GuiGraphics graphics, int x, int y, double mouseX, double mouseY, float partialTicks) {
        ...
        while( lines.hasNext() ) {
            ...
            if( this.bordered ) {
                this.renderLineBordered(graphics, line, x, y, lastLine);
            } else if( this.shadow ) {
                this.renderLineShadow(graphics, line, x, y, lastLine);
            }

            this.setColor(this.getTextColorKey(DISABLED_COLOR, HOVER_COLOR, this.currColorId, null, null), false);
            this.renderLine(graphics, line, x, y, lastLine);
    ...
    public void fromJson(IGui gui, GuiDefinition guiDef, JsonObject data) {
        ...
        //TODO: figure out how to load custom font
//        this.fontRenderer = Font.loadFont(data).get(Minecraft.getInstance());
    ...
    protected void renderLine(GuiGraphics stack, FormattedText s, int x, int y, boolean lastLine) {
        ...
        s.visit((style, str) -> {
            Component tc = Component.literal(str).withStyle(style);
            this.fontRenderer.drawInBatch(stack, tc, mx.getValue(), y, this.currColor);
